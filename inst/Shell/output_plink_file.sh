...
then
		...
		exit 1
			...
			mv $dataFolder"Force-Allele1-"$input"-HRC.txt" tmp/
			...
			plink  --bfile clean/TEMP1 --update-chr $dataFolder"Chromosome-"$input"-HRC.txt" --make-bed --out clean/TEMP2
			plink  --bfile clean/TEMP2 --update-map $dataFolder"Position-"$input"-HRC.txt" --make-bed --out clean/TEMP3
			...
			plink  --bfile clean/TEMP5  --keep-allele-order --list-duplicate-vars ids-only --reference-allele tmp/Force-Allele1-$input-HRC.txt --make-bed --out clean/TEMP6
...
if [ -f "$TEMP6_dupvar" ] && [ $(wc -l <"$TEMP6_dupvar") -ge 0 ]
then
...
then
	...
	exit 1
		   ...
		   gawk -f $updateDuplicates clean/TEMP_dupliPed.ped > clean/TEMP_dupliPed_2.ped
...
then
	...
	echo "VCF successfully created : $filename"
fi
fi
# 			rm -R tmp/ clean/
#
...
# 	echo "ERROR : No such file plink2VCF.sh line 119"
# 	exit 1
# fi
#

 ## compress into vcf.gz
 # bgzip -c tmp/DATA_updated_vcf.vcf.gz  > $filename-vcf.vcf.gz

  # bcftools norm --check-ref x -f $fastaFile tmp/DATA_updated_vcf.vcf.gz -o tmp/check_aa # SEUL A NE PAS NE FONCTIONNE PAS
  # vcftools --gzvcf tmp/DATA_updated_vcf.vcf.gz --out $filename-vcf

# filename=$filename-vcf.vcf
# if [[ ! -f "$filename" ]];
# then
# 	echo "ERROR : No such file plink2VCF.sh line 130"
# 	exit 1
# else
# 	echo "VCF successfully created : $filename-vcf.vcf"
# fi
